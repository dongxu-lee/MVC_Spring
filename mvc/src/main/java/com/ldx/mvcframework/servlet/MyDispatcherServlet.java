package com.ldx.mvcframework.servlet;
...
import com.ldx.mvcframework.pojo.Handler;
...
public class MyDispatcherServlet extends HttpServlet {
    ...
    private List<String> classNames = new ArrayList<>(); //缓存扫描到的类名
    ...
    private void doAutowired() {
        ...
        // 遍历ioc所有对象，查看对象中的字段，是否有@MyAutowired注解，如果有需要维护依赖注入关系
        for (Map.Entry<String, Object> entry : ioc.entrySet()) {
            // 获取Bean对象中的字段信息
            Field[] declaredFields = entry.getValue().getClass().getDeclaredFields();
            // 遍历判断处理
            for (int i = 0; i < declaredFields.length; i++) {
                Field declaredFiled = declaredFields[i];
                if (!declaredFiled.isAnnotationPresent(MyAutowired.class)) {
                    continue;
                }

                // 有注解
                MyAutowired annotation = declaredFiled.getAnnotation(MyAutowired.class);
                String beanName = annotation.value();
                if ("".equals(beanName.trim())) {
                    // 没有配置id，需要根据当前字段类型注入（接口注入）
                    beanName = declaredFiled.getType().getName();
                }
                ...
                try {
                    declaredFiled.set(entry.getValue(), ioc.get(beanName));
        ...
        try {
            for (int i = 0; i < classNames.size(); i++) {
                ...
                if (aClass.isAnnotationPresent(MyController.class)) {
                    // controller的id不做过多处理，不取value了。直接首字母小写，然后保存
                    ...
                    // service层往往都是有接口的，面向接口开发，
                    //此时再以接口名为id，放入一份对象到ioc中，便于后期根据接口类型注入
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        // 根据上述数组长度创建一个新的数组（参数数组，是要传入发射调用的）
        Object[] paraVlaues = new Object[parameterTypes.length];

        // 一下就是为了向参数数组中塞值，而且还要保证参数的顺序和方法中形参顺序一致
